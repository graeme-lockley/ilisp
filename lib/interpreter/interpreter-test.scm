...
(Unit.test "eval variable"
    ...
    (Unit.assert-signal (Interpreter.eval env 'y) (proc (s) (do
        (Unit.assert-equals (*builtin*.pair-car signal) 'UnboundVariable)
        (Unit.assert-equals (map-get (*builtin*.pair-cdr signal) :name) 'y)
        (Unit.assert-equals (map-get (*builtin*.pair-cdr signal) :procedure) 'lookup-variable-name)
    )))
)
...
(Unit.test "eval assignment"
    ...
    (Unit.assert-signal (Interpreter.eval env (list 'set! 'b "bye")) (proc (s) (do
        (Unit.assert-equals (*builtin*.pair-car signal) 'UnboundVariable)
        (Unit.assert-equals (map-get (*builtin*.pair-cdr signal) :name) 'b)
        (Unit.assert-equals (map-get (*builtin*.pair-cdr signal) :procedure) 'set-variable-value!)
    )))
)
