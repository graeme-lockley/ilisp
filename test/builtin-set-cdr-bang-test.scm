...
(Unit.test "set-cdr!"
    (Unit.assert-equals (*builtin*.set-cdr! (*builtin*.mutable-pair 1 2) 3) (*builtin*.pair 1 3))
)
...
(Unit.test "arg count signal"
    ...
    (Unit.assert-signal-name (*builtin*.set-cdr! (*builtin*.mutable-pair 1 2)) 'ExpectedArgumentCount)
    (Unit.assert-signal-name (*builtin*.set-cdr! (*builtin*.mutable-pair 1 2) 10 "asd") 'ExpectedArgumentCount)
)
